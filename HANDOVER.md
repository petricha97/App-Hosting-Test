...
In progress:
- **GitHub Agent** dispatched: commit the M6-T2 work on `feat/m6-t2-emails-admin` (branch doesn't exist yet — working tree changes are sitting directly on `prototype`; GitHub Agent needs to decide/execute the right branch handling), merge to `prototype` (--no-ff, smoke-check, merge log). NEVER main.

Next steps, in order:
12. Commit + merge M6-T2 → prototype. NEVER main.
13. Then M6-T3 (lifecycle triggers & audience segmentation) through the full loop (Research → Design → Implement → Review → Security → QA → merge).
14. Then M6-T4 (email designer via shared block engine) through the full loop.
15. **STOP before M7** (reporting) per user instruction — leave M6 fully closed/merged and M7 flagged ready-to-start, do not begin M7-T1.
