...
export function OrganizationForm() {
  ...
  const [foundOrg, setFoundOrg] = useState<{ id: string; name: string } | null>(
    null,
  );
  ...
  // M8-T1 §4: an invite-token signup never picks/creates an org here — the
  // invitation already assigns org + role. This step only needs to make
  // sure a Firebase Auth account exists, then hands off to /invite/{token}
  // to complete acceptance (mirrors join-organization-dialog.tsx's
  // ?code=... redirect pattern, D7).
  const inviteToken = store.prefilledInviteToken;
  ...
  const form = useForm<OrganizationValues>({
    ...
    defaultValues: {
      ...
      organizationName:
        domain && !isPersonal ? formatDomainForDisplay(domain) : "",
  ...
  // M8-T1 §4: creates the Firebase Auth account (email/password method;
  // Google sign-in already created it in credentials-form) then hands off
  // to /invite/{token}, which owns the actual accept call + state handling
  // (success / EMAIL_MISMATCH / invalid) — a single source of truth shared
  // with the "already had an account" acceptance path.
  const onAcceptInviteContinue = async () => {
    setLoading(true);
    setError(null);
    try {
      ...
      if (store.authMethod === "email") {
        ...
        if (store.name)
          await updateProfile(firebaseUser, { displayName: store.name });
      ...
      <Card className="w-full max-w-md">
        ...
        <CardContent className="space-y-4">
          ...
          <Button
            className="w-full"
            disabled={loading}
            onClick={() => void onAcceptInviteContinue()}
          >
            {loading ? (
              <>
            ...
            {action === "create" && (
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>
                      {isPersonal ? "Workspace" : "Organization"} name
                    </FormLabel>
                    <FormControl>
                      <Input
