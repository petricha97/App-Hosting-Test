...
- **Permissions:** every **mutating route and API read** in M5 (manual registration, CSV export, check-in config GET/PATCH, team-member create/revoke, scan resolve/confirm, roster/abandoned API reads) gates on session → org → `getAdminEventForOrganization` → `write:events` (403 / 404-IDOR per M1–M3 convention). **Server PAGES** (roster, abandoned tab, check-in config, dashboard scanner page shells) gate on **org membership** (`getDashboardScope`) for viewing — the repo's established read-surface convention (see 2026-07-13 clarification below). The public scanner authenticates via a **scanner access code → short-lived signed session token** (T4 decision below) — never a dashboard session.
- **Clarification 2026-07-13 (SEC finding L-4 reconciliation — spec amended, not code):** this spec originally said "every admin surface" gates `write:events`; the shipped code gates the M5 *server pages* on org membership only, with `write:events` on all mutations and API reads (including CSV export). The spec is hereby amended to record the shipped behavior as intended, for four reasons: (1) it is the repo-wide read-surface convention every M0–M4 page already follows — M5 pages diverging to `write:events` would have been the inconsistency; (2) it matches the Cvent product model: `users.html` defines a **Viewer** role whose purpose is read-only access to event data *including attendee lists*, so same-org read-only members seeing the roster/check-in pages is intended behavior, not a leak — cross-org isolation and the 404-IDOR contract are unaffected; (3) the material PII-egress channels stay strictly gated — bulk CSV export and every mutation require `write:events`; (4) IAM is still mock until **M8-T1**, so page-level `write:events` today would not map to a real Viewer/Editor distinction and would be reworked anyway. **Forward obligation for M8-T1 (role → permission matrix):** decide per-screen Viewer visibility of attendee PII — specifically whether Viewers see full roster emails or a masked variant (the abandoned tab already masks to domain-only) — and whether Viewers gain a read-only export. Until M8-T1, exposure is bounded to same-org members viewing read-only pages, accepted as convention.
